...
App.prototype.xhr = function (options) {
    ...
    if (options.load) {
        ...
        loadArea.addClass("loading");
        loadArea.css("padding-top", app.getDim()[1] / 3 + "px");
...
App.prototype.getFormData = function (formData) {
    ...
    for (var id in formData) {
        ...
        if ((!value || value.trim() === "") && formData[id].required) {
            elem.focus();
            app.showMessage(formData[id].message);
            app.scrollTo(id);
            return;
        }
        if(sign === "+" && parseFloat(value) < 0){
            elem.focus();
            app.showMessage("Field should have positive value");
            app.scrollTo(id);
            return; 
        }
        var obj = {};
