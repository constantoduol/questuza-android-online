...
App.prototype.loadCats = function (cats,displayArea,type,filter) {
    ...
    for (var x = 1; x < cats.length + 1; x++) {
        var name = cats[x - 1];
        var width = (app.getDim()[0]*0.95);
        var font_size = 20;
        var cont = {font_size : font_size,width : width,name : name,filter : filter};
        ...
        app.addCategory(displayArea,cont,clickHandler);
    }
};

App.prototype.addCategory = function (displayArea,cont,clickHandler){
...
App.prototype.sale = function(options){
    ...
    function appendItem(){
        ...
        //<td>" + priceSpan + "</td>
        tr.append("<td>" + prodSpan + "</td><td>" + qtySpan + "</td><td>" + subSpan + "</td>");
        var td = $("<td>");
        td.append(img);
        tr.append(td);
        saleArea.append(tr);
